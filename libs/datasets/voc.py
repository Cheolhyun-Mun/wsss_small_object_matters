...
class VOCAug(_BaseDataset):
    ...
    def _set_files(self):
        ...
        if self.split in ["train", "train_aug", "trainval", "train_aug_edam", "train_aug_eps", "train_aug_irn", "train_aug_nsrom", "train_aug_bana", "train_aug_rib", "train_aug_adv", "train_aug_drs", "train_aug_modi","train_aug_bbam","train_aug_cda","val"]:
            file_list = osp.join(
            ...
            if self.split == "val":
                file_list = osp.join(
                self.root, "ImageSets/SegmentationAug", "val_inst.txt"
                )
                file_list = tuple(open(file_list, "r"))
                file_list = [id_.rstrip().split(" ") for id_ in file_list]
                _, self.inst_labels = list(zip(*file_list))
        else:
            raise ValueError("Invalid split name: {}".format(self.split))
    ...
    def _load_data(self, index):
        ...
        label = np.asarray(Image.open(label_path), dtype=np.int32)
        if self.split == "val":
            inst_label_path = osp.join(self.root, self.inst_labels[index][1:])
            inst_label = np.asarray(Image.open(inst_label_path), dtype=np.int32)
            return image_id, image, label, inst_label
        return image_id, image, label
